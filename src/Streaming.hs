...
{- | Map effects of one monad to another with a transformation.
...
hoist phi s = loop phi s
  where
    Builder{..} = monadBuilder
    loop :: forall f m n r . (Control.Monad m, Control.Functor f) =>
      (forall x . m x #-> n x) -> Stream f m r #-> Stream f n r
    loop phi stream = stream & \case
      Return r -> Return r
      Effect m -> Effect $ phi $ Control.fmap (loop phi) m
...
> let noteBeginning text x = putStrLn text Control.>> Control.return text
